...
class NutanixPrismComputeUtilitySpec extends Specification {
	...
	void "listImagesV4 calls the vmm V4 content images endpoint and normalizes to the V3 shape"() {
		...
		then:
		1 * client.callJsonApi(authConfig.apiUrl, 'api/vmm/v4.3/content/images', authConfig.username, authConfig.password, _, 'GET') >> ServiceResponse.success([
				data    : [[
						extId                 : 'image-1',
						name                  : 'CentOS Image',
						type                  : 'DISK_IMAGE',
	...
	void "getImageV4 calls the vmm V4 content images endpoint for a single image and normalizes to the V3 shape"() {
		...
		then:
		1 * client.callJsonApi(authConfig.apiUrl, 'api/vmm/v4.3/content/images/image-1', authConfig.username, authConfig.password, _, 'GET') >> ServiceResponse.success([
	...
	void "createImageV4 posts a UrlSource body when given a sourceUri and normalizes the task reference"() {
		...
		then:
		1 * client.callJsonApi(authConfig.apiUrl, 'api/vmm/v4.3/content/images', authConfig.username, authConfig.password, {
	...
	void "deleteImageV4 calls DELETE on the vmm V4 content images endpoint and normalizes the task reference"() {
		...
		then:
		1 * client.callJsonApi(authConfig.apiUrl, 'api/vmm/v4.3/content/images/image-1', authConfig.username, authConfig.password, _, 'DELETE') >> ServiceResponse.success([data: [extId: 'task-3']])
		result.success
		result.data.status.execution_context.task_uuid == 'task-3'
	}

	void "listTemplates calls the vmm V4 content templates endpoint and fetches full vmSpec per template"() {
		given:
		def client = Mock(HttpApiClient)
		...
		then:
		1 * client.callJsonApi(authConfig.apiUrl, 'api/vmm/v4.3/content/templates', authConfig.username, authConfig.password, _, 'GET') >> ServiceResponse.success([
				data: [[extId: 'template-1', templateName: 'CentOS Template', templateVersionSpec: [vmSpec: null]]]
		])
		1 * client.callJsonApi(authConfig.apiUrl, 'api/vmm/v4.3/content/templates/template-1', authConfig.username, authConfig.password, _, 'GET') >> ServiceResponse.success([
	...
	void "createVmFromTemplate posts a deploy action to the vmm V4 content templates endpoint and normalizes the task reference"() {
		given:
		def client = Mock(HttpApiClient)
		def runConfig = [
		...
		then:
		1 * client.callJsonApi(authConfig.apiUrl, 'api/vmm/v4.3/content/templates/template-1/$actions/deploy', authConfig.username, authConfig.password, {
