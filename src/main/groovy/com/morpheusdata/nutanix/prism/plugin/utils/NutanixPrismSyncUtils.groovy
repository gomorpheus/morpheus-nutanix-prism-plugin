	...
	static syncVolumes(locationOrServer, ArrayList externalVolumes, Cloud cloud, MorpheusContext morpheusContext, opts=[:] ) {
		...
		try {
			...
			syncLists.updateList?.each { updateMap ->
				...
				def displayOrder = volume.device_properties?.disk_address?.device_index
				if(displayOrder && existingVolume.displayOrder != displayOrder) {
					existingVolume.displayOrder = displayOrder
					save = true
				}
		...
		try {
			...
			if (matchIpAddress) {
				...
				matchFunction = { morpheusItem, Map cloudItem ->
					//TODO:: this could be removed now
